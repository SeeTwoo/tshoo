...
size_t	skip_whitespaces(char const *s) {
	size_t	i = 0;
...
size_t	skip_to_next_block(char const *s) {
	size_t	i = 0;
...
t_seg	*new_segment_node(char *s, size_t len) {
...
t_seg	*get_segments(char *raw_line, t_key_value *aliases_list) {
	t_seg	*head;
	t_seg	*tail;
	char	*temp = raw_line;
	char	*alias;
...
char	*build_line(t_seg *segments) {
	char	*dest;
	char	*walker;
	size_t	dest_len = 0;
	t_seg	*temp = segments;
...
char	*new_aliasing(char *raw_line, t_key_value *aliases_list) {
	t_seg	*segments;
	char	*aliased_line;
